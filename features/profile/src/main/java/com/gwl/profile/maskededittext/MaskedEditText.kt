package com.gwl.profile.maskededittext
...
import com.gwl.core.MaterialEditText

class MaskedEditText : MaterialEditText, TextWatcher {
    ...
    constructor(
        ...
        allowedChars =
            attributes.getString(R.styleable.MaskedEditText_allowed_chars)
        deniedChars =
            attributes.getString(R.styleable.MaskedEditText_denied_chars)
        val enableImeAction = attributes.getBoolean(
            R.styleable.MaskedEditText_enable_ime_action,
            false
        )
        val representation =
        ...
        keepHint =
            attributes.getBoolean(R.styleable.MaskedEditText_keep_hint, false)
        cleanUp()
        ...
        for (i in 0 until mask!!.length) {
            ...
            if (mtrv != -1) {
                if (mtrv < rawText!!.length()) {
                    ...
                    hint?.get(maskToRaw[i])?.let { ssb.append(it) }
                }
            } else {
                ssb.append(mask!![i])
            }
